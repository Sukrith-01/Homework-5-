...
def question7():
    ...
    # type: explain_string
    answers['(i) Best classifier, explain'] = "In the current circumstances, each group of classifiers is the best option after weighing the context. C1 both has low recall and FPR so, C2 only has a recall of 50% but a very high FPR and no data for C2 for directly comparing with it. The best classification algorithm is the one that is specifically designed for the situation and takes into account the both true positive and true negative costs."
...
def question8():
    ...
    answers['p-range'] = 'Solve the inequality 2*(0.1*p)/(0.1+p) < 0.15 for p'
    return answers
...
def question9():
    ...
    answers['(i) metrics'] = {
        'recall': 80 / (80 + 70),          
        'precision': 80 / (80 + 50),       
        'F-measure': 2 * (80 / (80 + 50) * 80 / (80 + 70)) / (80 / (80 + 50) + 80 / (80 + 70)),
